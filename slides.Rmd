...
`r chunk_reveal(chunk_name = "shortestpath", break_type = "user", display_type = "both")`

```{r shortestpath, include = F}
p1 = net %>%
  activate(nodes) %>%
  st_as_sf() %>% 
  slice(1)
...
net %>% 
  convert(to_spatial_shortest_paths, 1, 9) -> #BREAK
  net_sp 
...
net %>% 
  morph(to_spatial_shortest_paths, 1, c(9, 10)) %>% #BREAK
...
`r chunk_reveal(chunk_name = "line_graph2", break_type = "user", display_type = "both", width_left = "55%", width_right = "42%", title = "###Second order binary adjacency", split = 50)`
